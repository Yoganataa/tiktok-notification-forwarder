...
export class UserMappingRepository extends BaseRepository {
  ...
  async updateTelegramTopic(
    ...
    // Check if the record exists first or do an update
    const sql = `
      UPDATE user_mapping
      SET telegram_topic_id = $2, updated_at = CURRENT_TIMESTAMP
      WHERE username = $1
      RETURNING *
    `;
    ...
    if (result) {
        ...
        logger.warn(`Failed to update Telegram Topic for ${username}. User mapping might not exist.`);
    }
  }
