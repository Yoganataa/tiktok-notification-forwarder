...
import { LIVE_PATTERNS, TIKTOK_URL_PATTERN, EMBED_COLORS } from '../../core/constants';
import {
  ContentType,
  TikTokUsername,
  NotificationData,
...
export class DiscordNotificationService {
  ...
  extractNotification(message: Message): NotificationData | null {
    ...
    if (message.embeds.length > 0) {
      ...
      if (embedResult.username) {
          username = embedResult.username;
          if (embedResult.url) url = embedResult.url;
      }
    }

    // 2. Try Components (Buttons)
    if (!username && message.components.length > 0) {
      const componentResult = this.extractFromComponents(message);
      if (componentResult.username) {
          username = componentResult.username;
    ...
    for (const row of message.components) {
      if ('components' in row) {
        for (const component of row.components) {
          // @ts-ignore - discord.js types can be tricky with union types of components
